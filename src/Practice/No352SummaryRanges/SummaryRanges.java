package Practice.No352SummaryRanges;

//        给你一个由非负整数a1, a2, ..., an 组成的数据流输入，请你将到目前为止看到的数字总结为不相交的区间列表。
//        实现 SummaryRanges 类：
//        SummaryRanges() 使用一个空数据流初始化对象。
//        void addNum(int val) 向数据流中加入整数 val 。
//        int[][] getIntervals() 以不相交区间[starti, endi] 的列表形式返回对数据流中整数的总结。
//
//        示例：
//        输入：
//        ["SummaryRanges", "addNum", "getIntervals", "addNum", "getIntervals", "addNum", "getIntervals", "addNum", "getIntervals", "addNum", "getIntervals"]
//        [[], [1], [], [3], [], [7], [], [2], [], [6], []]
//
//        输出：
//        [null, null, [[1, 1]], null, [[1, 1], [3, 3]], null, [[1, 1], [3, 3], [7, 7]], null, [[1, 3], [7, 7]], null, [[1, 3], [6, 7]]]
//
//        解释：
//        SummaryRanges summaryRanges = new SummaryRanges();
//        summaryRanges.addNum(1);      // arr = [1]
//        summaryRanges.getIntervals(); // 返回 [[1, 1]]
//        summaryRanges.addNum(3);      // arr = [1, 3]
//        summaryRanges.getIntervals(); // 返回 [[1, 1], [3, 3]]
//        summaryRanges.addNum(7);      // arr = [1, 3, 7]
//        summaryRanges.getIntervals(); // 返回 [[1, 1], [3, 3], [7, 7]]
//        summaryRanges.addNum(2);      // arr = [1, 2, 3, 7]
//        summaryRanges.getIntervals(); // 返回 [[1, 3], [7, 7]]
//        summaryRanges.addNum(6);      // arr = [1, 2, 3, 6, 7]
//        summaryRanges.getIntervals(); // 返回 [[1, 3], [6, 7]]


import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

public class SummaryRanges {
//    执行用时：312 ms, 在所有 Java 提交中击败了5.60%的用户
//    内存消耗：43.8 MB, 在所有 Java 提交中击败了47.20%的用户
   List<Integer> list;
    public SummaryRanges() {
        list = new ArrayList<>();
    }
    public void addNum(int val) {
        list.add(val);
    }
    public int[][] getIntervals() {
        if (list.size()==1){
            return new int[][]{{list.get(0),list.get(0)}};
        }
        Collections.sort(list);
        List<int[]> intevals = new ArrayList<>();
        int l = list.get(0);
        for (int i=1;i<list.size();i++){
            if (list.get(i)-list.get(i-1)>1){
                intevals.add(new int[]{l,list.get(i-1)});
                l=list.get(i);
            }
        }
        intevals.add(new int[]{l,list.get(list.size()-1)});
        return intevals.toArray(new int[intevals.size()][]);
    }

//    private int[] arr;
//    public SummaryRanges() {
//        arr = new int[0];
//    }
//
//    public void addNum(int val) {
//        int lenArr = arr.length;
//        int[] newArr = new int[lenArr];
//        if (lenArr>1) {
//            for (int i = 0, j = 0; i < lenArr; i++, j++) {
//                if (arr[j]>val){
//                    newArr[i] = val;
//                    i++;
//                }
//                newArr[i] = arr[j];
//            }
//        }else {
//            newArr[0]=val;
//        }
//        arr = newArr;
//    }
//
//    public int[][] getIntervals() {
//        List<int[]> res = new ArrayList<>();
//        int l = arr[0];
//
//        for (int i=1;i<arr.length;i++){
//            if (arr[i]>arr[i-1]+1){
//                res.add(new int[]{l,arr[i-1]});
//                l = arr[i];
//            }
//        }
//        res.add(new int[]{l,arr[arr.length-1]});
//        return res.toArray(new int[res.size()][]);
//    }
}
